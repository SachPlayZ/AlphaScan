...
contract_address = os.getenv("LINKING_CONTRACT_ADDRESS")
w3 = Web3(Web3.HTTPProvider(os.getenv("RPC_URL")))
contract = w3.eth.contract(address=contract_address, abi=abi)


def og_balance(address: str):
...
def buy_token(address: str, token_ticker: str, amount_in_og: float):
    token_ticker = token_ticker.upper()
    private_key = os.getenv("PRIVATE_KEY")
    if not private_key:
        raise ValueError("Private key not found in environment variables")
    ...
    if token_ticker == "DEAL":
        gas_estimate = token_contract.functions.buy().estimate_gas(
            {"from": public_key, "value": w3.to_wei(amount_in_og, "ether")}
        )
    else:
