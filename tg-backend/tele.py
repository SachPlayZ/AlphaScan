...
def generate(prompt: str):
    ...
    while retry_count < max_retries:
        try:
            client = OpenAI(
                base_url=os.getenv("OG_ROUTER_URL", "https://router-api-testnet.integratenetwork.work/v1"),
            ...
            response = client.chat.completions.create(
                ...
                model=os.getenv("OG_MODEL"),
                stream=False,
            )
            output = response.choices[0].message.content
            # Strip reasoning block if model emits one (e.g. <think>...</think>)
            if "</think>" in output:
...
async def analyse_texts(queue: List[Dict], user_id: str) -> Any:
    ...
    for token in tg_alpha:
        ...
        if token["sentiment"] == "positive":
            await log_action(
                "Check 0G Balance [Alpha is positive so we need to buy using 0G]",
                token,
                "Checking 0G balance",
                user_id
            )
            if not get_og_native_balance(user_id):
                await log_action(
                    "Check 0G Balance", token, "0G balance is zero", user_id
                )
                return
        ...
        if token["sentiment"] == "positive":
            try:
                ...
                if balance > 0:
                    ...
                    warning(f"Insufficient 0G balance for user {user_id} to buy {token['token']}")
                    await log_action(
                        "Check 0G Balance",
                        token,
                        "Insufficient 0G balance for purchase",
                        user_id
                    )
            except Exception as e:
                error(f"Error checking 0G balance for user {user_id}: {str(e)}")
                raise HTTPException(
                    status_code=500,
                    detail=f"Failed to check 0G balance: {str(e)}"
                )
