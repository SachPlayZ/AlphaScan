...
export interface WalletData {
  ...
  ogBalance: number; // Native 0G token balance
...
export function getOrCreateWallet(): WalletData {
  ...
  const walletData: WalletData = {
    ...
    ogBalance: 0, // Initial 0G token balance
    transactions: [],
  };
...
/**
 * Updates the native 0G token balance in the wallet
 * @param wallet Current wallet data
 * @returns Updated wallet data with current 0G balance
 */
export async function updateOGBalance(
  wallet: WalletData
): Promise<WalletData> {
  try {
    const provider = new ethers.JsonRpcProvider(
      ogTestnet.rpcUrls.default.http[0]
    );

    // Get the native token (0G) balance
...
/**
 * Sends native 0G tokens to another address
 * @param wallet Current wallet data
 * @param to Recipient address
 * @param amount Amount of 0G tokens to send
 * @param description Transaction description
 * @returns Object containing success status and transaction hash or error
 */
export async function sendOGTokens(
  wallet: WalletData,
  to: string,
  amount: number,
  description: string
  ...
  try {
    ...
    if (balance < amountInWei) {
      return { success: false, error: "Insufficient 0G balance" };
    }
    ...
    console.log("Sending transaction with parameters:", {
      to: to,
      value: ethers.formatEther(amountInWei) + " 0G",
    ...
    try {
      ...
      await updateOGBalance(updatedWallet);
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        errorMessage = "Network timeout. The 0G testnet might be congested";
      }
    }
 ...
 * @param wallet Current wallet data
 * @param to Recipient address
 * @param amount Amount of 0G to send
 * @returns Estimated gas in wei and in 0G
 */
export async function estimateTransactionGas(
  wallet: WalletData,
  to: string,
  amount: number
): Promise<{ gasInWei: bigint; gasIn0G: number }> {
  try {
    const provider = new ethers.JsonRpcProvider(
      ogTestnet.rpcUrls.default.http[0]
    );
    ...
    // Convert to 0G
    const gasIn0G = parseFloat(ethers.formatEther(gasInWei));
